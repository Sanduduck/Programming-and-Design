...
#define W              1280
#define H              720
#define ELEC_PLAYER_W       50
#define ELEC_PLAYER_H       50
...
static bool hit_player(float cx, float cy, float r) {
    ...
    if (nx > player.x + ELEC_PLAYER_W) nx = player.x + ELEC_PLAYER_W;
    if (ny < player.y)            ny = player.y;
    if (ny > player.y + ELEC_PLAYER_H) ny = player.y + ELEC_PLAYER_H;
...
void elec_update(float dt) {
    ...
    for (int i = 0; i < MAX_SPARKS; i++) {
        ...
        s->x += s->vx * dt;
        s->y += s->vy * dt;
        if (s->y > H + 60 || s->y < -60 || s->x < -60 || s->x > W + 60) {
            s->active = false;
            continue;
        }
