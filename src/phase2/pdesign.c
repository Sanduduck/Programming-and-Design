...
static bool player_over_cell(const PDesignCell *cell) {
    float px_center = player.x + (PLAYER_W / 2.0f);
...
static bool player_on_target(void) {
    ...
    float px = player.x;
    float py = player.y;
    return (px + PLAYER_W > cell->x && px < cell->x + CELL_W &&
            py + PLAYER_H > cell->y && py < cell->y + CELL_H);
}
...
// 패턴 디자인 화면을 그린다. (배경/바닥은 ui_play 공용 렌더에 맡기고 여기선 패턴만 그림)
