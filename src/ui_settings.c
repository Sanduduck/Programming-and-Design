...
// 무적 모드 토글
#define GOD_MODE_X  500
#define GOD_MODE_Y  330
#define GOD_MODE_W  220
#define GOD_MODE_H  42
...
static int hit_god_mode(int mx, int my) {
    return (mx >= GOD_MODE_X && mx < GOD_MODE_X + GOD_MODE_W &&
            my >= GOD_MODE_Y && my < GOD_MODE_Y + GOD_MODE_H);
}
...
void handle_settings_event(SDL_Event *e) {
    if (e->type == SDL_KEYDOWN) {
        switch (e->key.keysym.sym) {
            ...
            case SDLK_3:
            case SDLK_KP_3:
                g_settings.god_mode = !g_settings.god_mode;
                return;
        }
        ...
        if (hit_god_mode(mx, my)) {
            g_settings.god_mode = !g_settings.god_mode;
        } else if (hit_box(mx, my, BOX_LEFT_X)) {
            settings_set_scheme(CONTROL_WASD);
...
void draw_settings(SDL_Renderer *r) {
    ...
    // 무적 모드 토글
    draw_text(r, font_m, "무적 모드 (3)", SLIDER_LABEL_X, GOD_MODE_Y + 6, white);
    SDL_Rect god_mode_box = {
        GOD_MODE_X, GOD_MODE_Y, GOD_MODE_W, GOD_MODE_H
    };
    if (g_settings.god_mode) {
        SDL_SetRenderDrawColor(r, 0, 160, 100, 255);
    } else {
        SDL_SetRenderDrawColor(r, 90, 90, 90, 255);
    }
    SDL_RenderFillRect(r, &god_mode_box);
    SDL_SetRenderDrawColor(r, 255, 255, 255, 255);
    SDL_RenderDrawRect(r, &god_mode_box);
    draw_text_centered(r, font_m,
                       g_settings.god_mode ? "켜짐" : "꺼짐",
                       GOD_MODE_X, GOD_MODE_Y, GOD_MODE_W, GOD_MODE_H, white);

    // 조작키 안내
    draw_text_centered(r, font_m,
                       "조작키 설정   ( 1: WASD   2: 방향키 )",
                       0, 380, WINDOW_W, 30, white);
